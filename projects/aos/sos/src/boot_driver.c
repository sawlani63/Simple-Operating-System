...
int init_driver_irq_handling(seL4_IRQControl irq_control, seL4_Word irq, int level, seL4_Word badge)
{
    ...
    seL4_CPtr notification_cptr = cspace_alloc_slot(&clock_driver.cspace);
    ...
    err = cspace_mint(&clock_driver.cspace, notification_cptr, &cspace, clock_driver.ntfn, seL4_CanWrite, badge);
    if (err != seL4_NoError) {
        ZF_LOGE("Could not mint notification for timer irq");
        return -1;
    }
    err = seL4_IRQHandler_SetNotification(handler_cptr, clock_driver.ntfn);
    if (err != seL4_NoError) {
